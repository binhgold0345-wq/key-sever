...
# Hỗ trợ Key FREE (qua vượt link) và Key VIP (lưu trong keys.json)
...
# Định nghĩa thời gian theo loại key VIP
VIP_DURATIONS = {
    "1D": 24,
    "3D": 72,
    "7D": 168,
...
def load_keys():
    ...
    default_keys = {
        ...
        "TBTOOL_VIP_3D_001": {"type": "3D", "description": "Key VIP 3 ngày"},
        "TBTOOL_VIP_7D_001": {"type": "7D", "description": "Key VIP 7 ngày"},
        "TBTOOL_VIP_15D_001": {"type": "15D", "description": "Key VIP 15 ngày"},
...
def load_used_keys():
    """Tải danh sách key đã sử dụng (cả FREE và VIP)"""
...
def check_activation():
    """Kiểm tra trạng thái activation của key (FREE hoặc VIP)"""
    try:
        data = request.get_json()
        device_id = data.get('device_id')
        key = data.get('key')
        ...
        if key_hash in used_keys:
            ...
            return jsonify({
                ...
                "key_type": record.get("key_type", "FREE"),
                "is_vip": record.get("key_type", "FREE") != "FREE",
...
def delete_key():
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": f"Đã xóa key VIP: {key_to_delete}"
        })
